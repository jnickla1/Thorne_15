...
sel_methods = ['CGWL10y_sfUKCP', 'EBMKF_ta4','FaIR_comb_unB', 'FaIR_nonat_unB','GAM_AR1','GWI_anthro_SR15','Kal_flexLin','lowess1dg20wnc']
#already sorted
...
def plot_smoothed_histogram(ax, data0, title , leg=False, switch = 0):
...
plot_smoothed_histogram(axs[2,1], np.array(ssp_126e), "MPI ESM1-2-LR SSP126",leg=True,switch=1 ) 
...
for i,ax in enumerate([axs[1,0],axs[1,1]]):
    ...
    ax.text(cai[i]-2.3*wid_txt, caiht[i]*.75,"Too Early", fontsize=12,horizontalalignment='center',color='black')
    ax.arrow(cai[i]+2*wid_txt, caiht[i], wid_txt, 0,head_width=caiht[i]*3/12,head_length=wid_txt/2,facecolor='white',lw=2)
    ax.text(cai[i]+2.3*wid_txt, caiht[i]*.75,"Too Late", fontsize=12,horizontalalignment='center',color='black')
