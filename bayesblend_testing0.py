...
for p in range(30):
    ...
    loglik2 = -0.5 * np.log(2 * np.pi * sigma2**2) \
               - 0.5 * ((y_samp - mu2) ** 2) / sigma2**2


    # Step 2: Build Draws objects
    draws1 = bb.Draws(log_lik=loglik1, post_pred=post_pred1)
    draws2 = bb.Draws(log_lik=loglik2, post_pred=post_pred2)
    ...
    print(stack_model.weights['m1'][0,0])
...
#      np.nanmean(blended.post_pred_2d), 
#      np.nanstd(blended.post_pred_2d))
#somehow this is the wrong answer - even with appropriate weights it gives the incorrect answer
