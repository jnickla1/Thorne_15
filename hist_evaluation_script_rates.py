...
#CALCULATE RATES
    standard10 = results['cent10y']['LT_trend'][2] #retrospective
    standard10unc = results['cent10y']['LT_trend'][3] #retrospective
    ...
    closest_ind05 = np.argmin(np.abs(std_intp - 0.5))
    closest_ind10 = np.argmin(np.abs(std_intp - 1.0))
    ...
    uncert05a = 1.96* np.sqrt(std_intp10unc[closest_ind05 + int(4.5*inum)]**2 + std_intp10unc[closest_ind05 - int(4.5*inum)]**2)

    print(f'Decadal rate in {(closest_year05):.1f}  at 0.5°C: {(rate05a):.4f} ± {(uncert05a):.4f}')
    rate10a = std_intp10[closest_ind10 + int(5*inum)] - std_intp10[closest_ind10 - int(5*inum)]
    uncert10a = 1.96* np.sqrt(std_intp10unc[closest_ind10 + int(4.5*inum)]**2 + std_intp10unc[closest_ind10 - int(4.5*inum)]**2)
