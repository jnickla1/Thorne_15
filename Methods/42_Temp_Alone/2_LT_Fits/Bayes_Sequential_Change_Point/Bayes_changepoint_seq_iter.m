...
% ReadMe file
%
function Bayes_changepoint_seq_iter(loc_string)
...
idcs   = strfind(fileloc,'/');
dirpath =  fileloc(1:idcs(end)-1);

data = readtable(strcat(dirpath,'/input_data/',loc_string,'.csv'));
szd = size(data)

temperatures = NaN(szd(1)-49, szd(1)); 
uncertainties = NaN(szd(1)-49, szd(1));


for endi=50:(szd(1))
  

Y_complete=data.Anomaly(1:endi);
x_complete=data.Time(1:endi);
...
parameters = [d_min k_0 v_0 sig_0 k_max num_samp, 0];
...
[temp, unc] = plot_results(parameters, x, X, Y, Py, P, k);
    % Store the results in the preallocated matrices
temperatures(endi-49, 1:endi) = temp;
uncertainties(endi-49,1:endi) = unc;
...
csvwrite(strcat(dirpath,'/output/',loc_string,'BSCtemperatures.csv'), temperatures);
csvwrite(strcat(dirpath,'/output/',loc_string,'BSCuncertainty.csv'), uncertainties);