...
z1 = np.array([-0.01385267 , 4.53644205,  0.37530708  ,0.14695088 , 0.2838732 ])
...
def run_method(yrs, temps, uncert, model_run, experiment_type):
    ...
    if experiment_type != "historical":
        ...
        if (exp_attr[1]=='ESM1-2-LR'):
            ocean_data =Dataset(config.CLIMATE_DATA_PATH+"/ESM1-2-LR/combined/"+exp_attr[2].lower()+"_ocean_tas.nc", 'r').variables['ocn_ave_tas']
            ...
            ocean_data=Dataset(config.CLIMATE_DATA_PATH+"/NorESM_volc/BethkeEtAl2017/"+exp_attr[2].lower()+exp_attr[3]+"_ocean_tas.nc", 'r').variables['ocn_ave_tas']
            ocean_arr = ocean_data[:].__array__()
            OceanRec_sc0 =  average_every_n(ocean_arr[model_run,:], 12)
...
def KF_compute(years, temperature, match_series, z0):
    ...
    R = np.array([[ z0[2],z0[3]],[z0[3], z0[4]]])
