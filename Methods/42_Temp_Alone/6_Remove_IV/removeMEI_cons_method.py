...
def run_method(years, temperature, uncert, model_run, experiment_type):
    ...
    if experiment_type == "historical":
        ...
        end_yr = len(enso)+start_yr #otherse are left as NaN
        ...
        from 30y_etrend_method import run_method as run_method_30y_etrend
        smoothed_temp,_,_,_ = run_method_30y_etrend(years, temperature, uncert, model_run, experiment_type)
        
        
        if (exp_attr[1]=='ESM1-2-LR'):
            start_yr = 1
            data_loc = '/Users/JohnMatthew/climate_data/'+exp_attr[1]+'/combined/'+exp_attr[2].lower()+'_nino34_aave_tas.nc'
            ...
            start_yr = 22
            data_loc = '/Users/JohnMatthew/climate_data/'+exp_attr[1]+'_volc/BethkeEtAl2017/'+exp_attr[2].lower()+exp_attr[3]+'_nino34_aave_tas.nc'
            dataset = Dataset(data_loc, 'r')
            variable = dataset.variables['__xarray_dataarray_variable__']
            sims_tas = variable[:].__array__()
            ensoAuncor = average_every_n(sims_tas[model_run,8:-(12-8)], 12) #start in August/Sept
