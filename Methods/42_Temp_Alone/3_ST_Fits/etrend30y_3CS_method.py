...
def run_method(years, temperature, uncert, model_run, experiment_type):
    ...
    threeCS_assessed  = pd.read_csv(cur_path+"/current_warming_End_of_30-yr_trend_C3S.csv", header=[1])
    ...
    for i in range(len_trend-1, len(years)):
        ...
        means[i] = y_pred[-1]
        cyr = i+1850
        if (cyr>=2000 and cyr<=2022):
            means[i] =threeCS_assessed.iloc[cyr-2000,1]
        cis = confidence_interval(regX, y_pred, np.mean(regX), s_e, t_crit, len_trend ) #keep same CI as regular 30 yr trend
        ses[i] = cis[-1]
