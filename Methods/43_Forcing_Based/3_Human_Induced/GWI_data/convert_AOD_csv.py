...
def convert_csv(ssp, cmip_model):
    ...
    ts_file = config.CLIMATE_DATA_PATH+f"/{cmip_model}{cstr}/{ssp}_aod.nc"
    if cmip_model =="NorESM" and ssp == "hist":
        ts_file = config.CLIMATE_DATA_PATH+f"/{cmip_model}_volc/NorESM1-M-historical/{ssp}_aod.nc"
...
def combine_write(ssp0, cmip_model,volc=""):
    ...
    #df_solar = pd.read_csv(config.CODEBASE_PATH+"/Common_Data/toyKFmodelData8c.csv",header=None,index_col=0)
    #solar = df_solar[8]
