...
gwi_levels_retro0 = pd.read_csv(cur_path+"/anthropogenic-warming-assessment/results/Gillett_GMST_timeseries.csv", header=[0, 1])
gwi_levels_retro =gwi_levels_retro0.iloc[1:,] #blank row
gwi_r =gwi_levels_retro['Nat'].to_numpy() + gwi_levels_retro['Ant'].to_numpy() +0.0031
#gwi_levels_curr0 = pd.read_csv(cur_path+"/GWI_data/GWI_hist_only.csv", header=[0, 1])
#gwi_levels_curr =gwi_levels_curr0.iloc[1:,]
gwi_c =gwi_r #gwi_levels_curr['Tot'].to_numpy()
lyearr = np.shape(gwi_r)[0]
lyearc = lyearr #np.shape(gwi_c)[0]
lyear = lyearr + 1850 
ord_ind = [0,1,4,2,3]
min_fact = 4
...
def run_method(years, temperature, uncert, model_run, experiment_type):
    ...
    def empirical_mean(year_idx,k):
        k=1 #reasign no current estimate
        ...
        for i, yr in enumerate(year_idx):
            if (yr >=lyear or yr <1850):
                ...
                if (yr >=1950):
                    ...
                    resamps[i] = (cdists[yr-1950].ppf(uniform_samples))
            elif (k==1):
                if (yr >=1850):
                    uniform_samples = np.random.uniform(0, 1, size=n_samples)
                    resamps[i] = (rdists[yr-1850].ppf(uniform_samples))
        return resamps
