...
nbins=24
sdate=1850
data = np.genfromtxt(open("../../../Common_Data/toyKFmodelData8c.csv", "rb"),dtype=float, delimiter=',') #run as main
...
Khat=np.zeros((n_iters,4,3))      # smoothed gain or blending factor
...
#recombining
TOA_meas_artif = TOA_swout
TOA_meas_artif_var = (TOA_swout_low - TOA_swout_high)**2/4
...
def ekf_run(z,n_iter,retPs=False):
    ...
    for k in range(1,n_iter):
        ...
        Pminus[k] = np.matmul(np.matmul(F[k] ,P[k-1]), np.transpose(F[k])) +  np.array([[Q[0,0]  ,Q[0,1]  ,cM2[0,2], 0],
                                                                                        ...
                                                                                        [0       ,0       ,0       ,dfrA_float_var]])  #4x4 stuff
    ...
    if(False): #ignore RTS for now
        ...
        # measurement update
            try:
                ...
                Khat[k] =Khat[k+1]
            xhathat[k] = xhat[k]+np.matmul(Khat[k],(xhathat[k+1]- xhat[k] - compute_update(xhat[k],k)[:2]))
