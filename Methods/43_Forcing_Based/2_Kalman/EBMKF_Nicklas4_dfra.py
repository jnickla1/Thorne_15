...
def compute_slope_hidden(x2,ki, optdn=-1, lCo2n=-1,anthro_cloud=-101):
    ...
    if (lCo2n<=0):
        lCo2n= lCo2[k]
        tsik=tsi[k]
    [x,H,eng_bal,dfrA_float]= x2.T[0] #unpack completely
    inboundd=tsik*inbndf/(optdn+9.7279)*((dfaS+dfaA)+2*(dfaS*dfaA)*(x-T02)+dfaS*anthro_cloud/sw_in/0.9318/a_refl)
    outgoingd= outbndf*(4-powp1)*np.power(x, 3-powp1)*(1-B0*lCo2n) #0.0655
...
#following jacobian matrix conventions
    ...
    return np.array([[(1 + (inboundd-outgoingd - gad*(1-Cs/Cd))/heatCp ) , gad/Cd/heatCp, 0, inbound_ddfrA_float], \
                     [ (inboundd-outgoingd-gad*(1-Cs/Cd))*Cs/heatCp +gad*(1-Cs/Cd)+Cs  ,  ( 1 - (1-Cs/heatCp)*gad/Cd),0 , inbound_ddfrA_float*Cs],\
                     [inboundd-outgoingd,0,0,inbound_ddfrA_float],\
                     [0,0,0,1]]) #dfrA_float just randomly walks
...
def compute_update_hidden(x2,ki, optdn=-1, lCo2n=-1,anthro_cloud=-101): 
    k=int(ki)
    #breakpoint()
    [x,H,eng_bal,dfrA_float]= x2.T[0]    #we don't care about energy balance at last time point, so 3rd column is 0s
    #print(x)
    tsik=sw_in
    if (anthro_cloud<-100):
        anthro_cloud = anthro_clouds[k]
    ...
    #by convention addition of prior x, oc is not included
    Tchange=(inbound - outgoing - gad*(x-Teq1850 - oc + oc1850) )/heatCp
    return np.array([[x + Tchange, H + Tchange*Cs + gad*(x-Teq1850 - oc + oc1850) , inbound-outgoing,dfrA_float]]).T
...
for k in range(1,n_iters):
    temp_update = compute_update_hidden(
        np.array([[xblind[k-1,0,0],xblind[k-1,1,0],0,dfaA]]).T,k) #first compute the expected values of TOA radiation
    xblind[k]=temp_update[0:2]
    TOA_swout[k] = temp_update[2,0]
    TOA_swout_high[k] = compute_update_hidden(
        np.array([[xblind[k-1,0,0],xblind[k-1,1,0],0,dfaA + fdbkA_sigma/(sw_in*a_refl*g_refl)]]).T,k)[2,0]
...
def ekf_run(z,n_iter,retPs=False):
    ...
    for k in range(1,n_iter):
        temp_update = compute_update_hidden(
           np.array([[xblind2[k-1,0,0],xblind2[k-1,1,0],0,dfaA]]).T,k) #first compute the expected values of TOA radiation
        xblind2[k]=temp_update[0:2]

    

    # intial guesses
    xhat[0] = np.array([[Teq1850,oc_meas[0],0,dfaA]]).T
    P[0,:,:] = 1.0
    P[0,1,1] = 20
    Pminus[0,:,:] = P[0,:,:]
    ...
    for k in range(1,n_iter):
        ...
        Pminus[k] = np.matmul(np.matmul(F[k] ,P[k-1]), np.transpose(F[k])) +  np.array([[Q[0,0]   ,Q[0,1]   ,cM2[0,2] , 0],
                                                                                        ...
                                                                                        [0       ,0       ,0       ,dfrA_float_var/200]])  #4x4 stuff  dfrA_float_var

        # change 30 -> 10 to reduce additional measurement noise
        # change dfrA_float_var/60 due to overdispersion of CMIP

        S[k]= np.matmul(np.matmul(A ,Pminus[k]), np.transpose(A))+ 20* np.array([[Q[0,0]  ,Q[0,1]  ,cM2[0,2]],
                                                                                 [Q[1,0]  ,Q[1,1]  ,cM2[1,2]],
