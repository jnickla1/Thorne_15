...
#        self.CLIMATE_DATA_PATH = attribute1
#
#config = MyClass(os.path.expanduser('~/')+"climate_data")
...
def ext_timeser(timeseries, start_year=1850, end_year=2020, 
...
total_warming_se  = np.square(ekf.stdP.copy())
...
aer_temps_raw = Dataset(config.CLIMATE_DATA_PATH+"/attributable-warming_BCC-CSM2-MR/ts/aave_ts_Amon_BCC-CSM2-MR_hist-aer_r1i1p1f1_gn_185001-202012.nc", 'r').variables['ts']
...
aer_ohca_raw = Dataset(config.CLIMATE_DATA_PATH+"/attributable-warming_BCC-CSM2-MR/ohca/thetaoga_Omon_BCC-CSM2-MR_hist-aer_r1i1p1f1_gn_185001-202012.nc", 'r').variables['thetaoga']
aer_ohca = average_every_n(aer_ohca_raw[:].__array__(), 12)*5423.677
aer_ohca = aer_ohca - aer_ohca[0]
aer_toa_raw = Dataset(config.CLIMATE_DATA_PATH+"/attributable-warming_BCC-CSM2-MR/rt/aave_rtmt_Amon_BCC-CSM2-MR_hist-aer_r1i1p1f1_gn_185001-202012.nc", 'r').variables['rtmt']
...
new_observ_totalO3 = np.array([[ext_timeser(totalO3_temps,end_year=2014),ext_timeser(totalO3_ohca/ekf.zJ_from_W,end_year=2014) ,ext_timeser(totalO3_toa,end_year=2014)]]).T
...
GHG_temps_raw = Dataset(config.CLIMATE_DATA_PATH+"/attributable-warming_BCC-CSM2-MR/ts/aave_ts_Amon_BCC-CSM2-MR_hist-GHG_r1i1p1f1_gn_185001-202012.nc", 'r').variables['ts']
...
GHG_ohca_raw = Dataset(config.CLIMATE_DATA_PATH+"/attributable-warming_BCC-CSM2-MR/ohca/thetaoga_Omon_BCC-CSM2-MR_hist-GHG_r1i1p1f1_gn_185001-202012.nc", 'r').variables['thetaoga']
GHG_ohca = average_every_n(GHG_ohca_raw[:].__array__(), 12)*5423.677
GHG_ohca = GHG_ohca - GHG_ohca[0]
GHG_toa_raw = Dataset(config.CLIMATE_DATA_PATH+"/attributable-warming_BCC-CSM2-MR/rt/aave_rtmt_Amon_BCC-CSM2-MR_hist-GHG_r1i1p1f1_gn_185001-202012.nc", 'r').variables['rtmt']
...
ekf.lCo2= np.full(ekf.n_iters,np.log10(ekf.data[0,2]))
#aer set to 0 - dont change
nat_temps_raw = Dataset(config.CLIMATE_DATA_PATH+"/attributable-warming_BCC-CSM2-MR/ts/aave_ts_Amon_BCC-CSM2-MR_hist-nat_r1i1p1f1_gn_185001-202012.nc", 'r').variables['ts']
...
nat_ohca_raw = Dataset(config.CLIMATE_DATA_PATH+"/attributable-warming_BCC-CSM2-MR/ohca/thetaoga_Omon_BCC-CSM2-MR_hist-nat_r1i1p1f1_gn_185001-202012.nc", 'r').variables['thetaoga']
nat_ohca = average_every_n(nat_ohca_raw[:].__array__(), 12)*5423.677
nat_ohca = nat_ohca - nat_ohca[0]
nat_toa_raw = Dataset(config.CLIMATE_DATA_PATH+"/attributable-warming_BCC-CSM2-MR/rt/aave_rtmt_Amon_BCC-CSM2-MR_hist-nat_r1i1p1f1_gn_185001-202012.nc", 'r').variables['rtmt']
...
data = {
    'aer': (aer_warming, np.sqrt(aer_warming_se)),
    'ghg': (GHG_warming, np.sqrt(GHG_warming_se)),
    'ohf': (ohf_warming, np.sqrt(ohf_warming_se)),
    'ant': (anthro_warming, np.sqrt(anthro_warming_se)),
    'nat': (nat_warming, np.sqrt(nat_warming_se)),
    'tot': (total_warming, np.sqrt(total_warming_se))
}

np.savez_compressed(
    'Nicklas_GMST_timeseriesBCC-CSM2-MR.npz',
    aer_vals=aer_warming,
    aer_se=np.sqrt(aer_warming_se),
    ghg_vals=GHG_warming,
    ghg_se=np.sqrt(GHG_warming_se),
    ohf_vals=ohf_warming,
    ohf_se=np.sqrt(ohf_warming_se),
    anthro_vals=anthro_warming,
    anthro_se=np.sqrt(anthro_warming_se),
    nat_vals=nat_warming,
    nat_se=np.sqrt(nat_warming_se),
    tot_vals=total_warming,
    tot_se=np.sqrt(total_warming_se),
)

save_analytic_percentiles_from_mean_se("Nicklas_GMST_timeseriesBCC-CSM2-MR.csv", years, data)
