...
if __name__ == '__main__':
    ...
    ##        uncert10a = 1.96* np.sqrt(std_intp10unc[closest_ind10 + int(yrs_10wings*inum)]**2 + std_intp10unc[closest_ind10 - int(yrs_10wings*inum)]**2)
        ...
        frontend_rows = [
            {
                "level": f"{(0.5+sthresh):.1f}°C",
                "method": stdname,
                "year_cross": closest_year05,
                "rate": rate05a,
                "rate_uncert": uncert05a,
                "rate_window": yrs_05wings,
            },
            {
                "level": f"{(1.0+sthresh):.1f}°C",
                "method": stdname,
                "year_cross": closest_year10,
                "rate": rate10a,
                "rate_uncert": uncert10a,
                "rate_window": yrs_10wings,
            },
        ]
    ...
    ###sharp_blend_central = np.mean(sharpened_blended,axis=1)
        ...
        df_frontend["member"] = model_run
        all_frontend.append(df_frontend)
        
        #print(df_frontend)
        #print(time.process_time() - start)
        
    big = pd.concat(all_frontend, ignore_index=True)

    # Sanity check: are level+method identical across members?
    # (Raises if any member is missing a row or has a different ordering.)
    key_cols = ["level", "method"]
    ref_keys = all_frontend[0][key_cols].reset_index(drop=True)
    for i, df in enumerate(all_frontend[1:], start=1):
        if not ref_keys.equals(df[key_cols].reset_index(drop=True)):
            raise ValueError(f"Row order / keys differ for member {i}")

    # Group by (level, method) to aggregate
    group_cols = ["level", "method"]

    def mean_of_squares(x):
        # Used to combine rate_uncert properly as sqrt(mean(sigma^2) + var(rate))
        arr = np.asarray(x)
        return np.mean(arr**2)

    agg = big.groupby(group_cols).agg(
        year_cross_mean=("year_cross", "mean"),
        year_cross_sd=("year_cross", "std"),   # std across ensemble
        rate_mean=("rate", "mean"),
        rate_sd=("rate", "std"),               # std across ensemble
        rate_window_mean=("rate_window", "mean"),
        # We'll treat rate_uncert as a per-member std dev and combine in quadrature
        rate_uncert_var_mean=("rate_uncert", mean_of_squares),
    )

    # Combined columns
    agg["year_cross"] = agg["year_cross_mean"]
    agg["year_cross_sd"] = agg["year_cross_sd"]
    agg["rate"] = agg["rate_mean"]
    agg["rate_sd"] = agg["rate_sd"]
    agg["rate_window"] = agg["rate_window_mean"]
    ...
    agg = agg.drop(
        columns=[
            ...
            "rate_mean",
            "rate_window_mean",
            "rate_uncert_var_mean",
        ]
    )

    # Restore original (level, method) order from member 0
    order = ref_keys
    agg_reset = agg.reset_index()
    df_combined = order.merge(agg_reset, on=key_cols, how="left")
