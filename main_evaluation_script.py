...
regen=False #whether to execute all of the methods directly (True) or read from a pickle file to rapidly redraw the figures (False).
...
sel_methods = [  "CGWL10y_for_halfU","TheilSen_h7075" ,"FaIR_anthroA","FaIR_anthroA2","EBMKF_ta2"  ] #"EBMKF_ta",
...
#To read it back:
    ...
    ax4_handles=[]
    ax4_labels=[]
    df_results = pd.DataFrame(columns=['method_name', 'method_class','c/r','smooth_r','avg_unc.(1se)','#q<0.5', '#q<0.1', 'q_min', 'q_small5','log-likeli','RMS','bias','tlog-l','100log-l','l05','l10','bias50'])
    i=0
    ci=0
    labelcolors=[]
                ...
                df_results.loc[i]= [ method_name,short_method_class,labelcurr_or_retro,smooth_est/smooth_std,avg_uncert,
                                 qvals_count_yrs05,qvals_count_yrs01,  qvals_smallest,qvals_smallest5, np.nanmean(llikelihood), np.sqrt(np.nanmean((central_est-standard)**2)),
                                   np.nanmean(central_est-standard) , np.nansum(llikelihood),np.nansum(llikelihood[-100:-1]), np.exp(llikelihood[int(closest_year05)-1850]),
                                     np.exp(llikelihood[int(closest_year10)-1850]),np.nanmean(central_est[-50:]-standard[-50:])] 
                i=i+1
    ...
    ['43_Forcing_Based/3_Human_Induced', "4.3.2:\nAttributable\nWarming", 46.5,1999,2022], #99
    ['44_EarthModel_CGWL', "4.4:\nCombining\nESM Projections", 52,1998,2022]   #98
    ]
